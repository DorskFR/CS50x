...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_TOP or 
            self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_MID or 
            self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_BOT then
            gameState = 'win'

        elseif self.map:tileAt(self.x, self.y).id == CLOUD_LEFT or 
            self.map:tileAt(self.x + self.width - 1, self.y).id == CLOUD_LEFT or
            self.map:tileAt(self.x, self.y).id == CLOUD_RIGHT or 
            self.map:tileAt(self.x + self.width - 1, self.y).id == CLOUD_RIGHT then
            -- nothing we just bump a cloud

        elseif self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY or
...
function Player:checkLeftCollision()
    if self.dx < 0 then
        ...
        elseif self.map:tileAt(self.x - 1, self.y) == FLAG_TOP or 
            self.map:tileAt(self.x - 1, self.y) == FLAG_MID or 
            self.map:tileAt(self.x - 1, self.y) == FLAG_BOT then
            gameState = 'win'
        end
    end
end
...
function Player:checkRightCollision()
    if self.dx > 0 then
        ...
        elseif self.map:tileAt(self.x, self.y) == FLAG_TOP or 
            self.map:tileAt(self.x, self.y) == FLAG_MID or 
            self.map:tileAt(self.x, self.y) == FLAG_BOT then
            gameState = 'win'
        end
    end
end
